...
class Util {
    static getKey(socket, data) {
        try {
            const key = Util.parseJSON(data && data.toString());
            if (!key || !key.name) {
                return;
            }
    ...
    static handleKeyboard(callback) {
        readline.emitKeypressEvents(process.stdin);
        process.stdin.setRawMode(true);
        process.stdin.on('keypress', callback);
    }

    static throwMethodError(methodName, className) {
        const message = `Method "${methodName}" is not implemented in class "${className}"`;
        throw new Error(message);
    }

    static throwPropertyError(propertyName, className) {
        const message = `Property "${propertyName}" is not implemented in class "${className}"`;
        throw new Error(message);
    }
